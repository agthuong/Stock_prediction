...
def analyse_movement(df):
    ...
    fig, axes = plt.subplots(4, 1, figsize=(14, 16))#chỉnh sửa
...
def plot_stationary(df, df_stat):
    matplotlib.style.use('seaborn-darkgrid')
    fig, (ax2) = plt.subplots(1, 1, figsize=(12, 6)) #change ax2-ax1
    #ax1.plot(df['Close'], color='#DC76F7', linewidth=1)
    ax2.plot(df_stat['Close'], color='#DC76F7', linewidth=1)


    dates = [str(df.index.values[i]).split('T')[0] for i in range(len(df.index.values))]
    plot_labels = [item.get_text() for item in ax2.get_xticklabels()]
    ...
    #ax1.set_xticklabels(plot_labels_new)
    ax2.set_xticklabels(plot_labels_new)
    ...
    #ax1.set_title('Original closing price', fontsize=25)
    ax2.set_title('Stationary data of original closing price', fontsize=25)
